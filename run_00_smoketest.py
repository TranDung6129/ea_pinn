...
t0 = time.time()
E_batch = batch_oracle(np.array([[8.,1.,0.01],[1.5,4.,0.1],[3.5,1.5,0.05]]), n_jobs=3)
check("Batch FEM (3 calls, 3 workers)", len(E_batch)==3,
...
t0 = time.time()
gt = generate_ground_truth(n_grid=4, n_jobs=min(cfg.NUM_CPU_WORKERS, 4))
t_gt = time.time() - t0
